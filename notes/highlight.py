...
import keyword, tokenize, cgi, re, functools, logging
try:
    import builtins
except ImportError:
...
class HighlightError(tokenize.TokenError):
...
def analyze_python(source):
    index = 0
    tokens = analyze_python_chunk(source, index)
    while True:
        try:
...
def analyze_python_chunk(source, start=0):
...
def build_html_page(classified_text, template=default_html_template, **kwargs):
    'Create a complete HTML page with colorized source code'
    content = kwargs.copy()
    if 'css' not in content:
        content['css'] = default_css
    if 'title' not in content:
        content['title'] = 'python'
    processed_kwargs = {
        'css': '\n'.join(['%s %s' % item for item in content['css'].items()]),
        'title': cgi.escape(content['title']),
        'body': html_highlight(classified_text),
    }
